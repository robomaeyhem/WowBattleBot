...
public class Pokemon implements Serializable {
    ...
    public Pokemon(int id) {
        ...
        this.attack = calculateStats(id, Stats.ATTACK, level);
        this.defense = calculateStats(id, Stats.DEFENSE, level);
        this.spAttack = calculateStats(id, Stats.SP_ATTACK, level);
        this.spDefense = calculateStats(id, Stats.SP_DEFENSE, level);
        this.speed = calculateStats(id, Stats.SPEED, level);
        if (this.id != 292) {
            this.hp = calculateStats(id, Stats.HP, level);
        } else {
            this.hp = 1;
        }
        this.type1 = getFirstType(id);
        this.type2 = getSecondType(id);
        this.fainted = false;
    ...
    public void addExperience(int exp) {
        ...
        if (oldLevel != newLevel) {
            this.attack = Pokemon.calculateStats(this.id, Stats.ATTACK, level);
            this.defense = Pokemon.calculateStats(this.id, Stats.DEFENSE, level);
            this.spAttack = Pokemon.calculateStats(this.id, Stats.SP_ATTACK, level);
            this.spDefense = Pokemon.calculateStats(this.id, Stats.SP_DEFENSE, level);
            this.speed = Pokemon.calculateStats(this.id, Stats.SPEED, level);
            int oldHP = Pokemon.calculateStats(this.id, Stats.HP, oldLevel);
            int newHP = Pokemon.calculateStats(this.id, Stats.HP, newLevel);
            this.hp += (newHP - oldHP);
            this.maxHP = newHP;
        }
    }
    ...
    public void assignMoves() {
        ...
        if (compatableMoves.size() < 4) {
            ...
            for (int i = 1; i <= 4; i++) {
                ...
                while (move.getPower() == 0 || move.getAccuracy() == 0 || move.getCategory() == MoveCategory.STATUS) {
                    size++;
                    if (size > compatableMoves.size()) {
                        size = 0;
                    }
        ...
        SecureRandom rand = new SecureRandom();
        while (this.getMove1() == null || this.getMove1().getPower() == 0 || this.getMove1().getAccuracy() == 0 || this.getMove1().getCategory() == MoveCategory.STATUS) {
            int index = rand.nextInt(compatableMoves.size());
            this.setMove1(getMove(compatableMoves.get(index)));
        }
        while (this.getMove2() == null || this.getMove2().getPower() == 0 || this.getMove2().getAccuracy() == 0 || this.getMove2().getCategory() == MoveCategory.STATUS || this.getMove2().equals(this.getMove1())) {
            int index = rand.nextInt(compatableMoves.size());
            this.setMove2(getMove(compatableMoves.get(index)));
        }
        while (this.getMove3() == null || this.getMove3().getPower() == 0 || this.getMove3().getAccuracy() == 0 || this.getMove3().getCategory() == MoveCategory.STATUS || this.getMove3().equals(this.getMove1()) || this.getMove3().equals(this.getMove2())) {
            int index = rand.nextInt(compatableMoves.size());
            this.setMove3(getMove(compatableMoves.get(index)));
        }
        while (this.getMove4() == null || this.getMove4().getPower() == 0 || this.getMove4().getAccuracy() == 0 || this.getMove4().getCategory() == MoveCategory.STATUS || this.getMove4().equals(this.getMove1()) || this.getMove4().equals(this.getMove2()) || this.getMove4().equals(this.getMove3())) {
            int index = rand.nextInt(compatableMoves.size());
